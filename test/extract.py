...
async def fetch_data(device_serial_number: str, start_date: str, end_date: str, max_pages: int = 50):
    page_token = None
    all_data = []
    date_found = False  # Flag to check if we found any records for the date range
    ...
    try:
        ...
        for _ in range(max_pages):
            ...
            for record in payload[:-1]:  # Exclude the last item (page_token)
                ...
                if created_at:
                    ...
                    try:
                        # Convert to date and compare within the range
            ...
            if date_found:
                print(f"Records found for date range {start_date} to {end_date}: {len(all_data)}")
        ...
        if not all_data:
            print(f"No records found for date range {start_date} to {end_date} after checking {max_pages} pages.")
